...
public class FleePlayerAction extends LeafTask {
    ...
    public void DoTask() {
        if (blackboard.player.getPosition().dst(blackboard.actor.getPosition()) > AIConstants.FLEE_TO_RANGE){
        	...
        	blackboard.actor.body.applyForceToCenter(blackboard.actor.getPosition().cpy().sub(blackboard.player.getPosition()).nor().scl(blackboard.actor.body.getMass()*blackboard.actorSpeed), true);
        }
    }
